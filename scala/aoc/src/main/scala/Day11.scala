object Day11 {
	...
	def part2(s: String = puzzleInput, initialColor: Int = 1) = {
		var curr = Vector(0, 0)
		var painted: Set[Vector] = Set()
		var dir = Vector(0, 1)
		...
		val minX = color.keySet.map(_.x).min
		val maxX = color.keySet.map(_.x).max
		val minY = color.keySet.map(_.y).min
		val maxY = color.keySet.map(_.y).max

		for (y <- maxY to minY by -1) {
			val line = for (x <- minX to maxX) yield {
				color.get(Vector(x, y)).getOrElse(0) match {
					case 0 => "."
					case 1 => "#"
					case _ => ???
				}
