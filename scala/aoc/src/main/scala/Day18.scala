object Day18 {
    ...
    def reachableKeys(vault: Vault): Map[(Char, Pos), (Int, Set[Char])] = {
        import scala.collection.mutable.Queue

        var reachable: Map[(Char, Pos), (Int, Set[Char])] = Map()

        val from = vault.characterPos
		var visited: Set[Pos] = Set(from)
		val q: Queue[(Pos, Int, Set[Char])] = Queue((from, 0, Set()))
		while (!q.isEmpty) {
			val (p, dist, extraKeys) = q.dequeue()

            var addedKeys = extraKeys
            if (vault.keyPos.contains(p)) {
                val foundKey = vault.keyPos(p)
                reachable = reachable + ((foundKey, p) -> (dist, extraKeys))
                addedKeys = addedKeys + foundKey
            }

			for (neighbor <- vault.graph(p).filter(n => !visited.contains(n) && !vault.doorPos.keySet.contains(n))) {
				q.enqueue((neighbor, dist + 1, addedKeys))
    ...
    class PathFinder() {
        var memo: Map[(Pos, Set[Char]), Int] = Map()
        ...
        def shortestPath(vault: Vault): Int = {
            ...
            val keyDistances: Map[(Char, Pos), (Int, Set[Char])] = reachableKeys(vault)
            if (keyDistances.isEmpty) ???

            val possibleDistances = for (keyDist <- keyDistances) yield keyDist match {
                case ((key, pos), (dist, extraKeys)) =>
                    ...
                    val newVault = Vault(vault.graph, vault.keyPos -- removedKeyPos, vault.doorPos -- removedDoorPos, pos)
